...
if house_sale_file is None:
    forest_pickle = open('forest_house.pickle', 'rb')
    forest = pickle.load(forest_pickle)
    forest_pickle.close()

else:
    house_sale_df = pd.read_csv(house_sale_file)
    house_sale_df = house_sale_df.dropna()
    X=house_sale_df.drop(["SALES_PRICE"], axis=1)
    y=house_sale_df["SALES_PRICE"]
    ...
    score = "Test set accuracy: {:.2f}".format(forest.score(X_test, y_test))
